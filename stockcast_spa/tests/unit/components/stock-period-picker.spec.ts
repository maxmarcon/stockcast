import StockPeriodPicker, {Stock} from '@/components/stock-period-picker.ts'
...
describe('StockPeriodPicker', () => {
    ...
    it('initializes the tagged stock input', () => {
        expect(wrapper.findComponent(VueTagsInput).vm.$props.tags).toEqual([objectContaining({text: 'S1'})])
    ...
    describe('when a new tag is entered', () => {
        ...
        it('and the input event is emitted', () => {
            expect(wrapper.emitted('input')).toEqual([arrayContaining([objectContaining({stocks: [{text: 'S2'}]})])])
        })
    })
    ...
    describe('when a new date-from is entered', () => {
        ...
        it('and the input event is emitted', () => {
            expect(wrapper.emitted().input).toEqual([arrayContaining([objectContaining({dateFrom: parseISO('2020-01-01')})])])
        })
    })
    ...
    describe('when a new date-to is entered', () => {
        ...
        it('and the input event is emitted', () => {
            expect(wrapper.emitted().input).toEqual([arrayContaining([objectContaining({dateTo: parseISO('2020-01-01')})])])
        })
    })
    ...
    describe('when entering an ISIN in the stocks input field', () => {
        ...
        it('stores the result of the search including the matching ISIN', () => {
            expect(wrapper.vm.$data.autocompleteItems).toEqual([
                {symbol: "S1", name: "Stock 1", isin: "ISIN2"},
                {symbol: "S2", name: "Stock 2", isin: "ISIN2"}
            ])
        })
    })
    ...
    describe('when entering a FIGI in the stocks input field', () => {
        ...
        it('stores the result of the search including the matching FIGI', () => {
            expect(wrapper.vm.$data.autocompleteItems).toEqual([
                {symbol: "S1", name: "Stock 1", figi: "FIGI1"},
                {symbol: "S2", name: "Stock 2", figi: "FIGI1"}
            ])
        })
    })
